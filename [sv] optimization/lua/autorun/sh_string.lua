...
function string:Capitalize()
	local sFirstLetter = string.upper( string.sub(self, 1, 1) )
	return sFirstLetter .. string.sub( self, 2 )
end
...
function string.TimeSince( seconds )
	if seconds == nil then return "now" end
...
function string.HasNoSymbols( sText )
	return string.match( sText, "^[%w]+$" ) == sText
end
